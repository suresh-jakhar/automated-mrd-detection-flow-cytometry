...
async def predict_mrd(
    ...
    threshold: Optional[float] = Query(None, description="Optional override for reconstruction error threshold"),
    ...
    try:
        ...
        for file in files:
            ...
            try:
                ...
                total_rows += len(df)
                logger.info(f"Loaded {file.filename}: {len(df)} rows, {df.shape[1]} features")
        ...
        logger.info(f"Prediction completed in {result['processing_time_seconds']}s. MRD%: {result.get('mrd_pct', 'N/A')}")
...
async def predict_batch_with_labels(
    ...
    label_column: str = Query('mrd_label', description="Column name for true MRD% labels (optional)"),
):
    """
    ...
    try:
        ...
        X_preprocessed = predictor.preprocess(X)
        result = predictor.predict_mrd(X_preprocessed, batch_size=BATCH_SIZE, return_details=True)
